...
def create_shared_inner_splits():
    ...
    All models should reuse this exact file.
    """
    ...
    for repetition in range(1, N_REPETITIONS + 1):
        for outer_fold in range(1, N_OUTER_SPLITS + 1):
            ...
            if not outer_train_ids:
                raise ValueError(
                    f"No outer-training IDs for repetition={repetition}, "
    ...
    print(
        f"Created shared inner split file: "
        f"{inner_split_file}"
    )
...
def validate_inner_splits(
    ...
    for inner_fold in range(1, N_INNER_SPLITS + 1):
        ...
        if train_ids & val_ids:
            raise ValueError(
                "Inner train/validation overlap detected."
            )
...
def create_rnn_model_from_params(
    params,
    input_shape
):
    return build_rnn(
...
def make_inner_objective(
    ...
    """
    Uses the SAME saved inner folds for every Optuna trial.
    ...
    def objective(trial):
        ...
        for inner_fold_number, (
            ...
            # fit ONLY on inner-training genotypes.
            sx = StandardScaler().fit(
                X_inner_train
            )

            # Phenotypic standardization:
            # fit ONLY on inner-training phenotypes.
            sy = StandardScaler().fit(
                y_inner_train
            )

            X_inner_train_s = reshape_for_rnn(
                sx.transform(X_inner_train)
            )

            X_inner_val_s = reshape_for_rnn(
                sx.transform(X_inner_val)
            )

            y_inner_train_s = sy.transform(
                y_inner_train
            )

            y_inner_val_s = sy.transform(
                y_inner_val
            )
        ...
        # One Optuna objective value:
        # mean MSE across the SAME 5 inner folds.
...
# Fixed nested CV:
# 10 repetitions x 5 outer folds = 50 runs.
all_metrics = []
all_best_params = []
all_trial_records = []
...
for repetition in range(
    ...
    for outer_fold in range(
        ...
        try:
            ...
            # Hyperparameter search uses ONLY outer-training data.
            ...
            if (
                ...
                print(
                    f"No successful Optuna trial in "
            ...
            all_best_trial_records.append(
                best_record
            )

            # Final standardization:
            # fit ONLY on full outer-training data.
            ...
            X_outer_train_s = reshape_for_rnn(
                scaler_X.transform(
                    X_outer_train
                )
            )

            X_outer_test_s = reshape_for_rnn(
                scaler_X.transform(
                    X_outer_test
                )
            )

            y_outer_train_s = scaler_y.transform(
                y_outer_train
            )

            y_outer_test_s = scaler_y.transform(
                y_outer_test
            )
            ...
            # Outer-test is used ONLY once here.
            ...
            if (
                np.isnan(y_test_pred_s).any()
                or np.isinf(y_test_pred_s).any()
            ):
                print(
                    f"NaN predictions in repetition "
            ...
            # Convert phenotype predictions back to original scale.
...
print(
    f"\nCompleted {len(metrics_df)} successful "
    f"outer-fold evaluations out of "
