...
def create_mlp_model(trial, input_shape):
    """Variable-depth Dense network: 1-5 layers, each with its own
    units/activation. Search space is preserved from the original MLP.
    """
...
def create_mlp_model_from_params(params, input_shape):
    n_layers = params["n_layers"]

    layer_units = [
        params[f"units_layer_{i}"]
...
def make_inner_objective(
    ...
    """
    Optuna objective using the SAME saved inner folds for every trial.
    Outer-test data is never referenced here.
    """
    ...
    def objective(trial):
        ...
        for inner_fold_number, (
            ...
            # Genotypic standardization: fit only on inner-train.
            sx = StandardScaler().fit(X_inner_train)

            # Phenotypic standardization: fit only on inner-train phenotype.
...
# Fixed repeated nested CV: 10 repetitions x 5 outer folds = 50 runs
all_metrics = []
all_best_params = []
all_trial_records = []
...
for repetition in range(1, N_REPETITIONS + 1):
    ...
    for outer_fold in range(1, N_OUTER_SPLITS + 1):
        ...
        try:
            ...
            # Final scaling: fit ONLY on full outer-train.
            ...
            # Final model on ALL outer-training samples.
            set_seed(
                BASE_SEED
                + repetition * 10000
                + outer_fold * 100
            ...
            final_model = create_mlp_model_from_params(
                best_params,
                input_shape=(N_FEATURES,)
            )
