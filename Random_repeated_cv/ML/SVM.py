...
if len(id_to_idx) != len(ids):
    raise ValueError(
        "Permanent IDs must be unique."
    )

missing_outer_ids = (
    set(outer_split_df["ID"]) -
    set(id_to_idx)
)

missing_inner_ids = (
    set(inner_split_df["ID"]) -
    set(id_to_idx)
)
...
for repetition in range(
    ...
    for outer_fold in range(
        ...
        for inner_fold in sorted(
            ...
            # Both inner sets must be contained within
        ...
        # Best pipeline is automatically refitted on
