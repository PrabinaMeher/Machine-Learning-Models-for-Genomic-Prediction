...
split_file = os.path.join(
    dataset_dir, f"{feature_basename}_data_split.csv"
)
...
output_dir = os.path.join(
    dataset_dir, f"{feature_basename}_{MODEL_NAME}_out"
)
...
for repetition in range(1, N_REPETITIONS + 1):
    ...
    for outer_fold in range(1, N_OUTER_SPLITS + 1):
        ...
        # Map global dataset index -> position within outer training set
        ...
        for inner_fold in sorted(current_inner["Inner_Fold"].unique()):
            ...
            # Make sure inner IDs belong to this outer training set
            if not set(inner_train_idx_global).issubset(
                set(outer_train_idx)
            ):
                raise ValueError(
                    f"Inner training data contains samples outside "
                    f"outer training set: repetition={repetition}, "
                    f"outer_fold={outer_fold}, inner_fold={inner_fold}"
                )

            if not set(inner_val_idx_global).issubset(
                set(outer_train_idx)
            ):
                raise ValueError(
        ...
        # GridSearchCV(refit=True) refits the best model on
