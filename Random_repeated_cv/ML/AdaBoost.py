...
# Fixed split files generated previously
...
# Last column = phenotype/target
ids = df.iloc[:, 0].values
X_all = df.iloc[:, 1:-1].values
y_all = df.iloc[:, -1].values


# Load fixed OUTER split file
...
if not required_outer_cols.issubset(outer_split_df.columns):
    raise ValueError(
        f"Outer split file must contain columns: {required_outer_cols}"
    )


# Load fixed INNER split file

if not os.path.exists(inner_split_file):
    raise FileNotFoundError(
        f"Required inner split file not found:\n{inner_split_file}\n"
        "Create the shared inner split file once before running the models."
    )
...
if not required_inner_cols.issubset(inner_split_df.columns):
    raise ValueError(
        f"Inner split file must contain columns: {required_inner_cols}"
    )


# ID -> row position mapping

id_to_idx = {sample_id: i for i, sample_id in enumerate(ids)}

if len(id_to_idx) != len(ids):
    raise ValueError("Permanent IDs must be unique.")

# Check that every split ID exists in the current dataset
missing_outer_ids = set(outer_split_df["ID"]) - set(id_to_idx)
missing_inner_ids = set(inner_split_df["ID"]) - set(id_to_idx)
...
for repetition in range(1, N_REPETITIONS + 1):
    ...
    for outer_fold in range(1, N_OUTER_SPLITS + 1):
        ...
        train_ids = current_outer.loc[
            current_outer["Set"] == "train", "ID"
        ].tolist()

        test_ids = current_outer.loc[
            current_outer["Set"] == "test", "ID"
        ].tolist()

        if len(train_ids) == 0 or len(test_ids) == 0:
            raise ValueError(
                f"Missing train/test IDs for repetition={repetition}, "
                f"outer_fold={outer_fold}"
            )

        outer_train_idx = np.array(
            [id_to_idx[x] for x in train_ids], dtype=int
        )
        outer_test_idx = np.array(
            [id_to_idx[x] for x in test_ids], dtype=int
        )
        ...
        x_scaler = StandardScaler().fit(X_outer_train)

        X_outer_train_s = x_scaler.transform(X_outer_train)
        X_outer_test_s = x_scaler.transform(X_outer_test)

       
        # Build the FIXED inner CV from shared inner split file
        #
        # IMPORTANT:
        # GridSearchCV itself normally generates its own folds.
        # Here we explicitly give GridSearchCV the saved inner
        # train/validation folds so every model uses the SAME folds.
        ...
        if current_inner.empty:
            raise ValueError(
                f"No inner split found for repetition={repetition}, "
                f"outer_fold={outer_fold}"
            )

        cv_splits = []

        for inner_fold in sorted(current_inner["Inner_Fold"].unique()):
            ...
            inner_train_idx_global = np.array(
                [id_to_idx[x] for x in inner_train_ids], dtype=int
            )
            inner_val_idx_global = np.array(
                [id_to_idx[x] for x in inner_val_ids], dtype=int
            )

            # Convert global dataset indices to positions inside
            # X_outer_train_s because GridSearchCV receives only
            # X_outer_train_s.
            outer_train_position = {
                idx: pos for pos, idx in enumerate(outer_train_idx)
            }

            try:
                inner_train_idx = np.array(
                    [outer_train_position[x] for x in inner_train_idx_global],
                    dtype=int
                )
                inner_val_idx = np.array(
                    [outer_train_position[x] for x in inner_val_idx_global],
                    dtype=int
                )
            except KeyError as e:
                raise ValueError(
                    f"Inner split contains an ID outside the corresponding "
                    f"outer-training set: {e}"
                )

            cv_splits.append((inner_train_idx, inner_val_idx))
        ...
        ada_model = AdaBoostRegressor(random_state=rep_seed)
        ...
        # Evaluate ONCE on the fixed outer test set
...
# Aggregate results across all 50 outer runs
